...
const App: React.FC = () => {
  ...
  const addMessage = (role: 'user' | 'maya', text: string, imageUrl?: string) => {
    setMessages(prev => [...prev, { role, text, timestamp: new Date(), imageUrl }]);
  };

  const clearMemories = () => {
    if (confirm("Are you sure you want to clear Maya's memory of this session?")) {
      setMessages([]);
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          ...
          onerror: (e) => { addMessage('maya', "[System: Link failed.]"); stopSession(); },
  ...
  const handleDeepPrompt = async (text: string) => {
    ...
    try {
      ...
      if (mode === MayaMode.DEEP_THOUGHT) {
        ...
        addMessage('maya', response.text || "Thinking is hard sometimes.");
      } else if (mode === MayaMode.SEARCH) {
        // Use geolocation for Maps if possible
        let location = null;
        try {
        ...
        addMessage('maya', response.text || "Searching for answers...");
      } else if (mode === MayaMode.IMAGE) {
        response = await ai.models.generateContent({
          model: MODELS.IMAGE,
          contents: `Create an artistic visual interpretation of: ${text}. Witty and intuitive style.`,
        });
        ...
        addMessage('maya', "Here is a visual manifestation of that thought.", imageUrl);
      }
    } catch (err) {
      addMessage('maya', "My neural pathways are a bit tangled. Care to try again?");
    } finally {
      setIsThinking(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#0f172a] text-slate-100 p-4 gap-4 max-w-7xl mx-auto overflow-hidden">
      {/* Transcripts Panel */}
      <div className="flex-1 glass-card rounded-[2.5rem] p-6 flex flex-col h-[70vh] md:h-auto shadow-2xl relative overflow-hidden group border border-white/5 transition-all duration-700 hover:border-white/10">
        ...
        <div className="relative z-10 flex flex-col h-full">
          <div className="mb-6 border-b border-white/5 pb-5">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-white tracking-tight">Maya's Journal</h2>
                  <p className="text-[10px] uppercase tracking-[0.3em] text-indigo-400/70 font-bold">Neural Session Log</p>
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={exportJournal} className="p-2 hover:bg-white/5 rounded-xl text-slate-400 transition-colors" title="Export Log">
                ...
                <button onClick={clearMemories} className="p-2 hover:bg-red-500/10 rounded-xl text-slate-400 hover:text-red-400 transition-colors" title="Clear Memories">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" strokeWidth={2}/></svg>
                </button>
                <div className="px-4 py-1.5 rounded-full border border-white/5 bg-slate-900/50 flex items-center gap-2">
                  <span className={`h-2 w-2 rounded-full ${isActive ? 'bg-green-500 animate-pulse' : 'bg-slate-600'}`}></span>
                  <span className="text-[10px] uppercase font-black text-slate-400 tracking-wider">{isActive ? 'Live' : 'Off'}</span>
                </div>
              </div>
            </div>
            <div className="relative">
              <input type="text" placeholder="Recall a memory..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white/5 border border-white/5 rounded-2xl pl-10 pr-4 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500/50 transition-all text-slate-200" />
          ...
          <div className="flex-1 overflow-y-auto space-y-6 pr-2 scrollbar-hide relative pb-4">
            {filteredMessages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center opacity-50">
                <p className="text-lg font-semibold text-slate-300">A clean slate.</p>
                <p className="text-sm">Initiate a link to record your first thought.</p>
              </div>
            ) : (
                  ...
                  <div className={`relative max-w-[88%] min-w-[140px] rounded-3xl px-6 py-5 ${
                    ...
                    {m.imageUrl && (
                      <div className="mb-3 rounded-xl overflow-hidden border border-white/20 shadow-2xl">
                        <img src={m.imageUrl} alt="Generated by Maya" className="w-full h-auto object-cover max-h-64" />
                      </div>
                    )}
                    <p className={`text-[0.925rem] leading-[1.6] whitespace-pre-wrap font-medium ${m.text.startsWith('[System:') ? 'italic text-slate-400' : ''}`}>{m.text}</p>
                  </div>
                </div>
              ))
            )}
          ...
          <div className="mt-4 relative">
            <div className="relative flex items-center bg-slate-900/40 backdrop-blur-md border border-white/5 rounded-3xl p-2 shadow-2xl">
              <input type="text" placeholder={mode === MayaMode.LIVE ? (isActive ? "Listening..." : "Connect Voice to speak...") : `Share a thought in ${mode.toLowerCase()} mode...`} disabled={mode === MayaMode.LIVE || isThinking} onKeyDown={(e) => { if (e.key === 'Enter') { handleDeepPrompt(e.currentTarget.value); e.currentTarget.value = ''; } }} className="flex-1 bg-transparent border-none px-4 py-4 text-sm focus:outline-none placeholder:text-slate-600 font-medium disabled:opacity-20" />
      ...
      <div className="w-full md:w-96 flex flex-col gap-4">
        <div className={`glass-card rounded-[3rem] p-10 flex flex-col items-center justify-center flex-1 space-y-16 relative overflow-hidden transition-shadow duration-1000 border border-white/5 ${isActive ? 'maya-glow' : ''}`}>
          ...
          <div className="text-center space-y-3 z-20">
            ...
            <p className="text-slate-400 text-xs font-black uppercase tracking-[0.3em]">{isActive ? (isMayaSpeaking ? "Resonating" : "Listening") : "Neural Link Standby"}</p>
          </div>
        ...
        <div className="glass-card rounded-[2rem] p-4 flex flex-col gap-4 shadow-xl border border-white/5">
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              { id: MayaMode.IMAGE, label: 'Image', icon: 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z' }
            ].map(m => (
